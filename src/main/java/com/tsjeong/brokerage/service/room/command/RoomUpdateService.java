...
public class RoomUpdateService {
    ...
    public Room roomUpdate(
        ...
        if (transactionsDtos.isEmpty()) {
            throw ErrorCode.ENTITY_CONFLICT.build("하나의 방은 적어도 1개 이상의 거래 유형을 가져야 합니다.");
        }

        if (transactionsDtos.size() > RoomTransaction.MAX_PER_ROOM) {
            throw ErrorCode.ENTITY_CONFLICT.build("하나의 방은 최대 %d개의 거래 유형만 가질 수 있습니다.".formatted(RoomTransaction.MAX_PER_ROOM));
        }
...
//        roomTransactions.clear();
//        roomTransactions.addAll(roomTransactionUpdateService.updateRoomTransactionBy(room, transactionsDtos));
